        ...
        with torch.inference_mode():
            ...
            for t in range(args.max_steps):
                if t % args.Ta == 0:
                    ...
                    # predict
                    pred_raw_action = policy(cloud_data, actions = None, qpos = qpos, batch_size = 1).squeeze(0).cpu().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Tp', action = 'store', type = int, help = 'number of action pred', required = False, default = 16)
    parser.add_argument('--Ta', action = 'store', type = int, help = 'number of action execute', required = False, default = 16)
